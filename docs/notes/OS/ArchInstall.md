...
> [!info]
> 真正的 EFI 引导分区（ESP）其类型标识是全球统一的——这也就意味着，支持 UEFI 的主板肯定能识别到 ESP 分区。  
> 但`fdisk`创建的分区，其默认类型是 Linux Filesystem. 尽管也能用来启动 Arch，但终归是 Linux 专属——Windows 不一定能识别，一些有点年头的主板也不一定能识别。
